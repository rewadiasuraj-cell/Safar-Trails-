...
export const SeasonalTripsSection: React.FC<SeasonalTripsSectionProps> = ({
  ...
  return (
    <section id="seasonal-trips-section" className="w-full py-16 lg:py-24 bg-white border-t border-stone-100">
      <div className="w-full max-w-7xl xl:max-w-[1440px] 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
          <div>
            ...
            <p className="mt-2 text-stone-500 text-sm sm:text-base max-w-2xl font-normal">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {seasonalTripsData.map((season) => (
            <div
              key={season.id}
              className="bg-[#FAF9F6] rounded-2xl overflow-hidden border border-stone-200/80 shadow-xs hover:shadow-lg transition-all flex flex-col justify-between"
            >
              <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {season.destinations.map((d, i) => (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-md bg-white hover:bg-black hover:text-white text-stone-800 text-xs font-semibold border border-stone-200 transition-colors cursor-pointer"
                        >
                          {d}
                        </button>
                      ))}
              ...
              <div className="p-4 sm:p-5 pt-0">
                <button
                  ...
                  className="w-full h-10 min-h-[40px] px-2.5 sm:px-3 rounded-xl bg-black hover:bg-stone-800 text-white font-bold text-[11px] sm:text-xs uppercase tracking-wide sm:tracking-wider transition-colors inline-flex items-center justify-center gap-1.5 cursor-pointer text-center whitespace-nowrap select-none"
                >
